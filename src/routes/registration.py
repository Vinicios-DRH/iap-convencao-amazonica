...
def inscricao():
    ...
    if status == "embreve":
        return render_template("registration/inscricoes_em_breve.html")

    if status == "suspensas":
        return render_template("registration/inscricoes_suspensas.html")
    ...
    if form.validate_on_submit():
        if form.has_kids_u5.data == "sim" and not (form.kids_u5_names.data or "").strip():
            ...
            return render_template("registration/inscricao.html", form=form)
